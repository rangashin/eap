            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                    ...
                    @if (in_array($scholar->applicant->yearlevel, ['SHS', 'C']))
                        ...
                        @if (count($regis) == 0)
                            <div class="flex items-center max-w-2xl mt-3">
                                <div class="py-4 bg-white hover:bg-gray-50">
                                    <p class="font-black text-xl text-gray-700">{{ 'No files.' }}</p>
                                </div>
                            </div>
                        @else
                            @foreach ($regis as $regi)
                                <div class="flex items-center max-w-2xl mt-4">
                                    ...
                                    <form action="{{ route('admin.scholar.regi-destroy', ['user_id' => $scholar->applicant_user_id, 'id' => $loop->index]) }}" method="post">
                                        @csrf
                                        <x-primary-button class="bg-red-700">Delete</x-primary-button>
                                    </form>
                                </div>
                            @endforeach
                        @endif
                    ...
                    @else
                        @foreach ($reports as $report)
                            <div class="flex items-center max-w-2xl mt-4">
                                <x-text-input class="block w-full mr-3 flex-1 bg-slate-200" type="text" value="{{ $report->name }}" disabled />
